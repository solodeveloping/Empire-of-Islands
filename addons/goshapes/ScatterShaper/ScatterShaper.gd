...
@export var max_instance_count: int = 10:
	set(value):
...
enum InstanciationType {
	INSTANCED,
	SIMPLE_MULTI_MESH,
	GRID_MULTI_MESH,
}

@export var instanciation_type: InstanciationType = InstanciationType.INSTANCED:
	set(value):
		instanciation_type = value
		emit_changed()

@export
var multimesh_grid_size := Vector3i(16, 16, 16):
	set(value):
		multimesh_grid_size = value
		emit_changed()

@export
var max_instance_count_per_multimesh: int = 512:
	set(value):
		max_instance_count_per_multimesh = value
		emit_changed()
...
class ScatterBuilder extends ShapeBuilder:
	...
	func build(data: GoshapeBuildData) -> void:
		...
		while x < max_x:
			...
			while z < max_z:
				z += inc
				if instance_count >= style.max_instance_count:
					return
				...
				instance_count += 1
				inst.name = "%s_%d" % [inst.name, instance_count]
	...
	func commit(data: GoshapeBuildData) -> void:
		...
		instances = instances.filter(func(element): return element != null)
		
		var grid_mm: GridMultiMesh
		var multimesh: MultiMeshInstance3D
		var mm: MultiMesh
		# TODO : handle mesh
		if style.instanciation_type == InstanciationType.SIMPLE_MULTI_MESH:
			multimesh = MultiMeshInstance3D.new()
			print("mm", multimesh)
			SceneUtils.add_child(parent, multimesh)
			mm = MultiMesh.new()
			multimesh.multimesh = mm
			mm.transform_format = MultiMesh.TRANSFORM_3D
			mm.instance_count = instances.size()
			var mesh: MeshInstance3D = SceneUtils.find_first_child_of_type_depth_first(
				instances[0],
				MeshInstance3D,
			)
			if !mesh:
				push_error("could not find a MeshInstance3D")
				return
			mm.mesh = mesh.mesh
			
			# TODO: AABB
			#var toto = AABB()
			#toto.expand(Vector2.ZERO)
			
			# TODO : handle multiple meshes
			# TODO : handle multiple meshes in meshes
		elif style.instanciation_type == InstanciationType.GRID_MULTI_MESH:
			grid_mm = GridMultiMesh.new()
			grid_mm.name = "GridMultiMesh"
			SceneUtils.add_child(parent, grid_mm)
			var mesh: MeshInstance3D = SceneUtils.find_first_child_of_type_depth_first(
				instances[0],
				MeshInstance3D,
			)
			...
			grid_mm.mesh = mesh.mesh
			grid_mm.grid_size = style.multimesh_grid_size
			grid_mm.max_instance_count_per_multimesh = style.max_instance_count_per_multimesh
		
		#print("instance count ", instances.size())
		
		# FIXME: null is not good for multimesh
		var i = 0
		for inst in instances:
			...
			if place_on_ground:
				...
				ray.to = parent.global_transform * Vector3(pos.x, -10000, pos.z)
				#ray.collision_mask = 0xFF & (~collision_layer)
				ray.collision_mask = collision_layer
			...
			if !style.instanciation_type or style.instanciation_type == InstanciationType.INSTANCED:
				SceneUtils.add_child(parent, inst, false)
			elif style.instanciation_type == InstanciationType.SIMPLE_MULTI_MESH:
				multimesh.multimesh.set_instance_transform(
					i,
					inst.transform
				)
				
				# TODO : handle static body
				# probably need a specific class
				
			elif style.instanciation_type == InstanciationType.GRID_MULTI_MESH:
				var body: StaticBody3D = SceneUtils.find_first_child_of_type_depth_first(
					inst,
					StaticBody3D
				)
				if body:
					...
					printerr("did not find a body")
				
				var area: Area3D = SceneUtils.find_first_child_of_type_depth_first(
					inst,
					Area3D
				)
				
				grid_mm.add_instance(
					inst.transform,
					body,
					area
				)
		...
		if style.instanciation_type == InstanciationType.GRID_MULTI_MESH:
			grid_mm.assign_aabbs()
