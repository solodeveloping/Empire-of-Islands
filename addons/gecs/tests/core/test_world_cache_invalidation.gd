...
## invalidate QueryBuilder's cached results.  Previously, the code only fired
## cache_invalidated when archetypes.size() changed, which meant moves between
## pre-existing archetypes left _cache_valid=true and returned stale entity
## arrays from execute().  (commit a00dd90, CACHE-01 optimisation)
func test_existing_archetype_move_invalidates_query_cache():
	# 1. Create two entities that establish two distinct archetypes:
	#    archetype {A}     and     archetype {A, B}
	var entity1 = Entity.new()
	var entity2 = Entity.new()
	entity1.add_component(C_TestA.new())
	entity2.add_component(C_TestA.new())
	...
	# 2. Run a query and cache it — only entity2 has both A and B.
	var query = world.query.with_all([C_TestA, C_TestB])
	var result1 = query.execute()
	assert_array(result1).has_size(1)
	assert_bool(result1.has(entity2)).is_true()

	# 3. Give entity1 component B → it moves from existing {A} to existing {A,B}.
	#    No new archetype is created, but the cached result must update.
	entity1.add_component(C_TestB.new())

	var result2 = query.execute()
	assert_array(result2).has_size(2)
	assert_bool(result2.has(entity1)).is_true()
	assert_bool(result2.has(entity2)).is_true()

	# 4. Remove component B from entity2 → moves from existing {A,B} to existing {A}.
	entity2.remove_component(C_TestB)

	var result3 = query.execute()
	assert_array(result3).has_size(1)
	assert_bool(result3.has(entity1)).is_true()
	assert_bool(result3.has(entity2)).is_false()
