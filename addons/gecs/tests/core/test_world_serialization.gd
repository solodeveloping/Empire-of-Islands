...
func test_basic_entity_serialization():
	...
	entity.add_component(
		C_SerializationTest.new(
			100, 9.99, "serialized", true, Vector2(3.0, 4.0), Vector3(5.0, 6.0, 7.0), Color.GREEN
		)
	)
	entity.add_component(
		C_Persistent.new("Hero", 10, 85.5, Vector2(50.0, 100.0), ["shield", "bow"])
	)
...
func test_multiple_entities_with_persistent_components():
	...
	for i in range(5):
		...
		entity.add_component(
			C_Persistent.new(
				"Player" + str(i), i + 1, 100.0 - i * 5, Vector2(i * 10, i * 20), ["item" + str(i)]
			)
		)
...
func test_binary_format_and_auto_detection():
	...
	entity.add_component(
		C_Persistent.new(
			"BinaryPlayer", 99, 88.8, Vector2(100.0, 200.0), ["sword", "shield", "potion"]
		)
	)
	...
	if text_file and binary_file:
		...
		print(
			"  Compression: ",
			"%.1f" % ((1.0 - float(binary_size) / float(text_size)) * 100),
			"% smaller"
		)
...
func test_prefab_entity_serialization():
	...
	assert_that(entity_data.scene_path).is_equal(
		"res://addons/gecs/tests/entities/e_prefab_test.tscn"
	)
...
func test_entity_level_serialize_config_override():
	...
	entity.add_component(
		C_Persistent.new("EntityPlayer", 50, 60.0, Vector2(90, 100), ["entity_item"])
	)
...
func test_config_hierarchy_priority():
	...
	other_entity.add_component(
		C_Persistent.new("Other", 60, 50.0, Vector2(110, 120), ["other_item"])
	)
